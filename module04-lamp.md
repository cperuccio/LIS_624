...
1. **Linux**: Linux is the operating system, and users interact with the operating system through the command line interface.
2. **Apache**: Apache is the web server. Web servers store files; because files can also store code, then web servers can run code, too. When a web browser requests access to retrieve or display a web server's files, the server can provide access (or indicate the error preventing access).
3. **MySQL**: MySQL is a relational databases that stores and organizes databases and data tables.
4. **PHP**: PHP is a server-side programming language that can retrieve data from MySQL and process the results as HTML that the web server can process.
...
#### Apache
1. To install Apache, I ran this command: `sudo apt install apache2`.
	1. `apache2` is the name of the package I want to install, but I can run `apt show apache2` if I want to learn more information about the package before installation.
2. Run the `systemctl` command to get the status of `apache2` once installed.
	1. Once I run `systemctl status apache2`, I want to see that the package is **enabled** and **active.
	2. If the package is **enabled**, then it will start automatically once I reboot the operating system (by way of running `systemctl`.
	3. If the package is **active**, it means that `apache2` is actually running.
...
1. To install both PHP and a package that creates the connection between PHP and Apache, run the `apt install` command: `sudo apt install php libapache2-mod-php`.
	1. Note that you can install multiple packages at the same time with the `apt install` command.
...
3. If Apache is running, we also need to check that PHP has been successfully installed to work with Apache. To do this, change to Apache's document root directory: `cd /var/www/html/
4. Then create a `.php` file: `sudo nano info.php`
	1. Add text to the file: 
		<?php	
		phpinfo();
		?>
5. If PHP is working with Apache as it should, we can test our public IP address with `info.php` added to the end of the IP address url.
	1. Delete that file after the test (security reasons): `sudo rm /var/www/html/info.php`
6. The URL of my IP address will default to a file, `index.html`. But if I want to provide PHP to Apache, then I'd like Apache to display `index.php` as my default. Here's how:
	...
	2. To do this, change my directory: `cd /etc/apache2/mods-available/`
	...
	7. We should see `Syntax Ok`, and then reload Apache and check the status.
...
5. In my regular user account, I created a database (`create database opacdb`) and added a table to my database. From there, I added records to the table.
6. To support the connection between PHP and MySQL, we need another module: `sudo apt install php-mysql`
...
8. PHP also needs to authenticate itself in order to connect to MySQL databases. To do so, add a `login.php` file to the document root's parent directory:
	...
	5. Add credentials to the `login.php` using `sudo nano login.php`. Here it is line by line:
		1. `<php // login.php`
		2. `$db_hostname = "localhost";`: remember, he hostname is the name of the regular user account I created in step 4!
		3. `$db_database = "opacdb";`: `opacdb` is the database I created in step 5.
		4. `$db_username = "opacuser";`: see step 4.
		5. `$db_password = "XXXXXXXXX";`: note the password created during step 4.
		6. `?>`
8. Next create a PHP file in our Apache root. 
	1. First change to the apache root: `cd /var/www/html`.
	2. Create a file in this root that interact with the database I created in step 5.
...
9. Final step: test the PHP syntax! If there are errors, a line number will be indicated:
	1. `sudo php -f /var/www/login.php`
	2. `sudo php -f /var/www/html/opac.php`
	3. View the site on my browser using the public IP address.
